...
return $fbcode;
	...
	public function atcw($pid, $quantity, $flg) {
		...
		if($this->config->get('config_fbcapidyad') && $pinfo) {
			...
			$fb_value = $this->tax->calculate($pinfo['price'], $pinfo['tax_class_id'], $this->config->get('config_tax'));
			$flag_name = 1;
 			...
 			if($rs) { 
				foreach ($rs as $pinfo) {
					...
					$contents[$cnt] = array(
						...
						"item_price" => $this->getcurval($pinfo['price']),
					);
				}
			}
